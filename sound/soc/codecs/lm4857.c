...
static unsigned int snd_soc_cache_read(struct snd_soc_codec *codec,
		unsigned int reg)
{
	unsigned int val;
	int ret;

	ret = snd_soc_read_cache(codec, reg, &val);
	if (ret)
		return -1;
...
static const struct snd_soc_dapm_widget lm4857_widgets[] = {
    SND_SOC_DAPM_INPUT("RIN"),
    SND_SOC_DAPM_INPUT("LIN"),

    SND_SOC_DAPM_OUTPUT("LLS");
    SND_SOC_DAPM_OUTPUT("RLS");
...
static const struct snd_kcontrol_new lm4857_controls[] = {
	...
	SOC_ENUM_EXT("Amp Mode", lm4857_mode_enum[0],
		lm4857_set_mode, lm4857_get_mode);
...
static int lm4857_probe(struct snd_soc_codec *codec)
{
	...
	codec->control_data = lm4857->i2s;

	return snd_soc_add_controls(codec, lm4857_controls,
				ARRAY_SIZE(lm4857_controls));
}

static int lm4857_remove(struct snd_soc_codec)
{
...
static struct snd_soc_codec_driver soc_codec_dev_lm4857 = {
	...
	.reg_cache_size = ARRAY_SIZE(lm4857_reg_defaults),
	.reg_word_size = sizeof(uint8_t),
	.reg_cache_default = lm4857_reg_defaults,
...
{
	struct lm4857 *lm4857 = i2c_get_clientdata(client);

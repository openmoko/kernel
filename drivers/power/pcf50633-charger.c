...
static irqreturn_t pcf50633_mbc_irq_handler(int irq, void *data)
{
	...
	if (mbc->pcf->pdata->mbc_event_callback)
		mbc->pcf->pdata->mbc_event_callback(mbc->pcf, irq - mbc->pcf->pdata->irq_base);
...
static int __devinit pcf50633_mbc_probe(struct platform_device *pdev)
{
	...
	/* Set up IRQ handlers */
	for (i = 0; i < PCF50633_MBC_NUM_IRQS; ++i) {
		ret = request_threaded_irq(mbc->irqs[i], NULL, pcf50633_mbc_irq_handler,
						0, pcf50633_mbc_irq_names[i], mbc);
		if (ret) {

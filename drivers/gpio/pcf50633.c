 ...
 * Copyright 2010-2011, Lars-Peter Clausen <lars@metafoo.de>
 * All rights reserved.
 *
		...
		int value)
{
	...
	u8 reg = PCF50633_REG_GPIOCFG(gpio);
		...
		enum pcf50633_gpio_config config)
{
	...
	if (gpio < 3) {
		mask = 1 << gpio;
		direction = (config == PCF50633_GPIO_CONFIG_INPUT) ? mask : 0;
		ret = pcf50633_reg_set_bit_mask(pcf, PCF50633_REG_GPIOCTL, mask,
				direction);
		if (ret)
			return ret;
...
int pcf50633_gpio_power_supply_set(struct pcf50633 *pcf,
		int gpio, int regulator, bool on)
{
	u8 reg, mask;
	...
	return pcf50633_reg_set_bit_mask(pcf, reg, mask, on ? mask : 0);
}
...
static int __devinit pcf50633_gpio_probe(struct platform_device *pdev)
{
	...
	chip->direction_input = pcf50633_gpio_direction_input;
	chip->direction_output = pcf50633_gpio_direction_output;
	chip->get = pcf50633_gpio_get_value;
	chip->set = pcf50633_gpio_set_value;

	chip->base = pcf->pdata->gpio_base;
	chip->ngpio = 4;
	chip->label = dev_name(pcf->dev);
	chip->can_sleep = 1;

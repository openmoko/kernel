...
static int __devinit pcf50633_input_probe(struct platform_device *pdev)
{
	...
	irq_released = platform_get_irq_byname(pdev, "released");
	if (irq_released <= 0) {
	...
	irq_pressed = platform_get_irq_byname(pdev, "pressed");
	if (irq_pressed <= 0) {
